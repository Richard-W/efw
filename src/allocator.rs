...
unsafe impl core::alloc::GlobalAlloc for Allocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        match efi::SystemTable::get().boot_services().allocate_pool(
            efi::bits::MemoryType::LoaderData,
            layout.size() + layout.align() + PTR_SIZE - 1,
        ) {
            Err(_) => 0 as _,
            Ok(raw) => {
                ...
                let aligned =
                    (unaligned + PTR_SIZE + layout.align() - 1) / layout.align() * layout.align();
